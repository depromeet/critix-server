...
public record ProjectFeedbackResponse(
    @Schema(description = "프로젝트 평가 정보", requiredMode = REQUIRED)
        ProjectEvaluation projectEvaluation) {

  @Schema(description = "프로젝트 평가 정보")
  public record ProjectEvaluation(
      @Schema(description = "프로젝트 이름", requiredMode = REQUIRED) String projectName,
      @Schema(
      ...
      @Schema(description = "페이지별 이슈 리스트", requiredMode = REQUIRED) List<PageIssue> pageIssues) {}

  @Schema(description = "페이지별 정보")
  public record PageIssue(
      @Schema(description = "페이지 번호", example = "1", requiredMode = REQUIRED) String pageNumber,
      @Schema(
              description = "이슈가 발생한 페이지 이미지 링크",
              example = "https://example.com/page1.png",
              requiredMode = REQUIRED)
          String imageLink,
      @Schema(description = "페이지 내 콘텐츠 이슈 리스트", requiredMode = REQUIRED)
          List<PageContentIssue> contents) {}

  @Schema(description = "페이지 콘텐츠 이슈 상세 정보")
  public record PageContentIssue(
      @Schema(description = "이슈 유형", example = "문장 오류", requiredMode = REQUIRED) String type,
      @Schema(
              description = "기존 문장",
